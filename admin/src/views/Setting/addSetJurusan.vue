                            ...
                            <div class="form-group">
                                ...
                                <select name="" id="" v-model="formData.nama" class="form-control" >
                                 <option value="" selected disabled>Pilih Dosen</option>
                                    <option v-for="(listjurusan,index) in listJurusan" :key="index"  :value="listjurusan.NOMOR"> {{listjurusan.JURUSAN}} </option>             
                                </select>
                            </div>
                            <div class="form-group">
...
export default {
  ...
  methods: {    
      async getDataJurusan(){
          ...
          let response = await axios.get('https://project.mis.pens.ac.id/mis129/API/DataMaster/jurusan.php?function=getJurusan',config)
      ...
      async store (){     
        ...
        let config = {
         ...
         let response = await axios.post(`https://project.mis.pens.ac.id/mis129/API/DataMaster/settting.php?function=addSetJurusan `,this.formData,config)
